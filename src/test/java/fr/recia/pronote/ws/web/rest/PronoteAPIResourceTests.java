...
public class PronoteAPIResourceTests extends AbstractPronoteWsApplicationTests {
    ...
    @Test
    void downloadedXmlShouldMatchXsd() throws Exception {

        File file = new File(
                Objects.requireNonNull(
                        PronoteAPIResourceTests.class.getResource("/xsd/ConteneurImportChiffre.xsd")
                ).toURI());
        ...
        MvcResult result = mockMvc.perform(get("/api/download/{uai}", uaiTest))
                .andExpect(status().isOk())
                .andExpect(header().exists(HttpHeaders.CONTENT_DISPOSITION))
                .andExpect(header().string(
                        HttpHeaders.CONTENT_DISPOSITION,
                        "attachment; filename=\"" + uaiTest + ".xml\""))
